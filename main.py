...
def load_credentials():
    if os.path.exists(USERS_FILE):
        try:
            ...
            return pd.DataFrame(columns=["username", "name", "password"]), {"usernames": {}}
    else:
        users_df = pd.DataFrame(columns=["username", "name", "password"])
        users_df.to_csv(USERS_FILE, index=False)
        return users_df, {"usernames": {}}
...
# --- Helper functions for reporting data ---
def load_reports_data():
    """Load and prepare reports data"""
    if os.path.exists(REPORTS_FILE):
        return pd.read_csv(REPORTS_FILE)
    else:
        df = pd.DataFrame(columns=["description", "category", "latitude", "longitude", "username", "before", "after", "timestamp"])
...
def load_initiatives_data():
    ...
    if os.path.exists(INITIATIVES_FILE):
        return pd.read_csv(INITIATIVES_FILE)
    else:
        df = pd.DataFrame(columns=["name", "description", "contact", "publisher", "timestamp"])
...
def load_points_data():
    ...
    if os.path.exists(POINTS_FILE):
        ...
        df = pd.DataFrame(columns=["username", "lifepoints"])
...
def load_responses_data():
    ...
    if os.path.exists(RESPONSES_FILE):
        ...
        df = pd.DataFrame(columns=["initiative", "participant", "contact", "timestamp"])
